...
class ControlNode(object):
	...
	#self.config.enable_stream(rs.stream.depth, 640, 240, rs.format.z16, 30)
	...
	if self.vel_state:
	    ...
	    indice=[j for j, x in enumerate(angles) if - 15 < x < 15]
	    to_right = [i for i, x in enumerate(angles) if x < -100]
            if len(indice)>0 :
	       error_dist=0.8-distances[indice[0]]
            else :
               error_dist=-1.0
	    
	    if error_dist>0.4 or self.bandera:

               if (not self.on_left_lane and to_left) or self.obst: 
	          self.on_left_lane = True
	          self.model_number = 2
                  self.bandera=True
	       elif self.on_left_lane and to_right:
	          for ang in to_right:
		     if 0.35 <=distances[ang] <= 0.80:
		         self.on_left_lane = False
		         self.model_number = 1
   			 self.bandera=True
		         break
            if 0<=error_dist<0.38 :
		 self.cont_error=int(((20.0/0.8)*error_dist)+60)
            if error_dist<0 :
		 self.cont_error=60
	...
	#depth_frame = frames.get_depth_frame()
		...
		"""if depth_frame:
		    dist = 0.01 * depth_frame.get_distance(120, 320)	    
		    if dist <= 1.5:
		    	self.obst = True
		    else:
		    	self.obst = False"""
