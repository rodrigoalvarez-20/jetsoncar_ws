...
class RCCarAuto(Node):

    def __init__(self):
        super().__init__("rc_car_auto")
        ...
        self.declare_parameter("navigation_topic", "/car/navigation")
        self.declare_parameter("use_navigation", 1)
        self.device = None
        self.rc_bridge = serial.Serial(
            self.get_parameter("bridge_port").value,
            int(self.get_parameter("bridge_baudrate").value),
            timeout=5,
        )

        self.navigation_sub = None
        # self.camera_bridge = None

        self.use_navigation = self.get_parameter("use_navigation").value == 1
        ...
        self.left_stick_drift = float(self.get_parameter("left_stick_drift").value)
        self.right_stick_drift = float(self.get_parameter("right_stick_drift").value)
        self.left_trigger_drift = float(self.get_parameter("left_trigger_drift").value)
    ...
    def listener_callback(self, data):
        ...
        try:
            ...
            if self.use_navigation:
                self.get_logger().info("Moving car...")
                steering_val, throttle_val = data.data
                throttle_val = min(
                    max(round(abs(throttle_val) * 1.5, 2), 0), 0.4
                )  # min(max(abs(round(throttle_val*1.2, 2)), 0), 0.5) #round(0.6 - throttle_val, 3)
                steering_val = round(steering_val, 2)

                self.get_logger().info(
                    "Data parsed: {} - {}".format(steering_val, throttle_val)
                )
                self.on_left_stick_x_changed(steering_val)
                # if steering_val < 0:
                #    self.on_left_stick_x_changed(min(steering_val*10, -1))
                # else:
                #    self.on_left_stick_x_changed(max(steering_val*10, 1))

                self.on_right_trigger(throttle_val)
    ...
    def connect_to_device(self):
        while not self.device:
            ...
            self.get_logger().info(
                "Se han encontrado {} dispositivos".format(len(device_infos))
            )
    ...
    def rescale_input(
        ...
        input_min_value=-1,
        input_max_value=1,
        rescale_min_value=-45,
        rescale_max_value=45,
    ):
        """
        ...
        """
        return (input_value - input_min_value) * (
            rescale_max_value - rescale_min_value
        ) / (input_max_value - input_min_value) + rescale_min_value

    def stop(self):
        pass

    def on_l1_button_press(self):
        self.use_navigation = not self.use_navigation
        self.get_logger().info("Navigation is {}".format(self.use_navigation))

    # def on_r1_button_press(self):
    #    self.is_recording = False
    #    txt_contents = ""
    #    self.get_logger().info(
    #        "Exporting {} items in queue".format(self.record_queue.qsize())
    #    )
    ...
    def on_right_trigger(self, value):
        ...
        if self._is_reverse_active:
            ...
            throttle_value = 90 - self.rescale_input(
                value_no_drift,
                input_min_value=0,
                # input_max_value=0.5,
                rescale_min_value=0,
                rescale_max_value=18,
            )
        else:
            throttle_value = 90 + self.rescale_input(
                value_no_drift,
        ...
        self.get_logger().debug(
            "right trigger changed: {} | {}".format(value_no_drift, throttle_value)
        )
    ...
    def on_left_stick_x_changed(self, left_stick_x):
        ...
        value_no_drift = (
            left_stick_x - self.left_stick_drift
            if left_stick_x >= 0
            else left_stick_x + self.left_stick_drift
        )
        ...
        if value_no_drift >= -0.2 and value_no_drift <= 0.2:
            ...
            steering_value = 90
            self.get_logger().debug(
                "Vehiculo centrado | {} | Joy Data: {}".format(
                    value_no_drift, steering_value
                )
            )
        elif value_no_drift < -0.2:
            self.get_logger().debug(
                "Vehiculo girando a la izquierda | {} | Joy Data: {}".format(
                    value_no_drift, steering_value
                )
            )
            ...
            self.get_logger().debug(
                "Vehiculo girando a la derecha | {} | Joy Data: {}".format(
                    value_no_drift, steering_value
                )
            )
            # En este caso, se tiene que reescalar el valor de 0 a 45 para poder restarlo o sumarlo a los valores de la direccion
            ...
            steering_value = 90
            self.get_logger().debug(
                "Vehiculo en estado desconocido | {}".format(value_no_drift)
            )
        # Enviar datos
    ...
    def __send_controls__(self, steering, throttle):
        ...
        if self.rc_bridge and self.rc_bridge.isOpen():
            ...
            self.get_logger().warning(
                "No Bridge connection to send command. Skipping..."
            )
